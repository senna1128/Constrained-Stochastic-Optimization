...
function main()
    ...
    if Berahas.verbose
        NumProb = 10
        decom = convert(Int64, floor(length(BerahasR)/NumProb))
        for i=1:decom
            path = string("../Solution/BerahasSQP", i, ".mat")
            ResultBerahas = BerahasR[(i-1)*NumProb+1:i*NumProb]
            write_matfile(path; ResultBerahas)
        end
        path = string("../Solution/BerahasSQP", decom+1, ".mat")
        ResultBerahas = BerahasR[decom*NumProb+1:end]
        write_matfile(path; ResultBerahas)
    end
