...
function main()
    ...
    AdapL1R = AdapL1Main(AdapL1, Prob)

    if AdapL1.verbose
        NumProb = 10
        decom = convert(Int64, floor(length(AdapL1R)/NumProb))
        for i=1:decom
            path = string("../Solution/AdapL1SQP", i, ".mat")
            ResultL1 = AdapL1R[(i-1)*NumProb+1:i*NumProb]
            write_matfile(path; ResultL1)
        end
        path = string("../Solution/AdapL1SQP", decom+1, ".mat")
        ResultL1 = AdapL1R[decom*NumProb+1:end]
        write_matfile(path; ResultL1)
    end
end
