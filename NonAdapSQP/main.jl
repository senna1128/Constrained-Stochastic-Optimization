...
function main()
    ...
    if NonAdap.verbose
        ...
        for i=1:decom
            path = string("../Solution/NonAdapSQP", i, ".mat")
            ResultNonAdap = NonAdapR[(i-1)*NumProb+1:i*NumProb]
            write_matfile(path; ResultNonAdap)
        end
        path = string("../Solution/NonAdapSQP", decom+1, ".mat")
        ResultNonAdap = NonAdapR[decom*NumProb+1:end]
        write_matfile(path; ResultNonAdap)
    end
end
