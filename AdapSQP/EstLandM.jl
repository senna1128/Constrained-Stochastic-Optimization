...
function EstLandM(nx,nlam,Xi,G_k,c_k,nab_xL_k,nab_x2L_k,nab_x2c_k,sigma,CovM,nu,alpha,kap_grad,p_grad,C_grad,MAX_SAMPLE = 100000)
    while true
        ...
        if Xi >= MAX_SAMPLE
            ...
            Quant4 = C_grad*log(8*nx/p_grad)/Quant3
            if Xi >= Quant4
                return bnab_xL_k, bM_k, Xi
            else
                Xi *= 1.5
