...
function AdapSQP(nlp,sigma,Max_Iter,EPS,nu,mu,epsilon,beta,rho,alpha_max,kap_grad,kap_f,p_grad,p_f,C_grad)
    nx = nlp.meta.nvar
    nlam = nlp.meta.ncon
    ...
    IdCon, IdSing, Xi = 1, 0, 1
    alpha_k, bnabL_mu_k, Quant, CountSam = alpha_max, zeros(nx+nlam), 0, 0
    nab_x2c_k = Array{Array}(undef, nlam)
    ...
    while min(eps, KKT[end]) > EPS && k < Max_Iter
        ...
        bnab_xL_k, bM_k, Xi = EstLandM(nx,nlam,Xi,G_k,c_k,nab_xL_k,nab_x2L_k,nab_x2c_k,sigma,CovM,nu,alpha_k,kap_grad,p_grad,C_grad)
        CountSam += Xi
        ...
        if IdSing == 1
            ...
            while true
                ...
                if mu > 1e8
                    break
                elseif Quant > -nu/2*norm([NewDir[1:nx];G_k*bnab_xL_k])^2 || norm(c_k) > norm(bnabL_mu_k)
                    mu *= rho
                else
                    break
                end
            ...
            Xi_f = min(C_grad*log(4/p_f)/Quant_com, 10000)
            CountSam += Xi_f
    ...
    if k == Max_Iter
        ...
        return X, Lam, KKT, floor(CountSam), Time, 1, 0
    end
end
